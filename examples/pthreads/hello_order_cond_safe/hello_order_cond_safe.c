...
int main(int argc, char* argv[]) {
  ...
  if (shared_data) {
    // Se crea un arreglo de punteros inicializados en valor 0
  ...
  if (threads && private_data) {
    for (uint64_t thread_number = 0; thread_number < shared_data->thread_count
      ...
      if (shared_data->greets[thread_number]) {
        // Dos formas de hacerlo, la comentada es con aritmetica de punteors
        // esta linea lo que hace es que incializa el primer valor en 0
