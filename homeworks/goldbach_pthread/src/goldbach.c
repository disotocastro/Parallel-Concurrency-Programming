...
int64_t goldbach_even(array_numbers_t* arr_input_stdin,
  ...
  sem_t* can_print, sem_t* next_thread) {
    ...
    for (size_t index_1 = 0; index_1 < arr_prime_numbers->count; index_1++) {
      for (size_t index_2 = index_1; index_2 < arr_prime_numbers->count; index_2++) {
        if (arr_prime_numbers->elements[index_1] && arr_prime_numbers->elements[index_2] &&
          (arr_prime_numbers->elements[index_1] + arr_prime_numbers->elements[index_2]) ==
          llabs(arr_input_stdin->elements[main_index])) {

          if (arr_input_stdin->elements[main_index] < 0) {
            if (array_append(arr_goldbach, arr_prime_numbers->elements[index_1]) != EXIT_SUCCESS) {
                fprintf(stderr, "Error: Could not add goldbach sums\n");
                return EXIT_FAILURE;
            }
            if (array_append(arr_goldbach, arr_prime_numbers->elements[index_2]) != EXIT_SUCCESS) {
                fprintf(stderr, "Error: Could not add goldbach sums\n");
                return EXIT_FAILURE;
            }
            goldbach_index += 2;
...
int64_t goldbach_odd(array_numbers_t* arr_input_stdin,
  ...
  for (int64_t index_1 = 0; index_1 < (int64_t)arr_prime_numbers->count; index_1++) {
    for (int64_t index_2 = index_1; index_2 < (int64_t)arr_prime_numbers->count; index_2++) {
      for (int64_t index_3 = index_2; index_3 < (int64_t)arr_prime_numbers->count; index_3++) {
